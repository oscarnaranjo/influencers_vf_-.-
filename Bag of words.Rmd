...
```{r}
data_tdm_matrix<-as.matrix(data_tdm2)
```
...
```{r}
barplot(frecuencia [1:10], col="#30e3dd", las=2, ylim=c(0, 250), main = "10 Palabras más repetidas", ylab="Frecuencia", cex.names = 0.8)
```
...
```{r}
word_associate(data_bag, match.string =c("run"), cloud.colors = c("green","blue"), network.plot = TRUE, wordcloud = TRUE)
```

#dendogram
#19)creo nuevo TDM con sparsity
```{r}
dendogram_tdm<-removeSparseTerms(data_tdm2, sparse=0.995)
```
